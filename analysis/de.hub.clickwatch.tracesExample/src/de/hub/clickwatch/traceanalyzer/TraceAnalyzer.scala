...
class TraceAnalyzer {
   ...
   * @param targetModel
   * 	A Resource in which the target class should be searched in
   */
  def which(targetClass : Class[_], srcObject : Traceable, setOfSourceModels : ResourceSet, targetModel : Resource) : EObject = {
    var bestCandiate : EObject = null
    var bestPathLength = -1
    var curPathLength = -1

    // possible candidates of the target class
    val candidates = getPossibleTargetCandidates(targetClass, targetModel)
