...
import Configuration.ProtocolType;
...
public class ConfigurationExample {
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			...
			try {
				Resource resource = resourceSet.createResource(URI.createURI("configuration2.xml"));
				...
				NetworkType network = ConfigurationFactory.eINSTANCE.createNetworkType();
				network.setName("Haha");
				network.setUpdateInterval(1000);
				NodeType node = ConfigurationFactory.eINSTANCE.createNodeType();
				node.setPort(BigInteger.valueOf(22));
				node.setProtocol(ProtocolType.TCP);
				node.setAddress("141.20.37.89");
				network.getNode().add(node);
				root.getNetwork().add(network);
