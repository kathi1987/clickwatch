...
public class ClickWatchServer implements IClickWatchServer {
		...
		if (resource != null) {
			...
			for (EObject eObject : resource.getContents()) {
				if (eObject instanceof DocumentRoot) {
					...
					for (ExperimentType experiment : configurationFile
							.getExperiments().getExperiment()) {
						// Set up the database
						this.setUpDatabase(experiment.getDatabase());
						
						// Create experiment
						this.createExperiment(experiment, experiment.getDatabase());
						// Set up the handler (?)
	 ...
	 *  
	 */
	private void createExperiment(ExperimentType pExperiment, DatabaseType pDatabase) {
		if (this.mServerDatabaseConnection != null) {
			...
			for (NodeType node : pExperiment.getNodes().getNode()) {
					...
					db.setUpDatabaseConnection(pDatabase.getUser(), pDatabase
							.getPassword(), pDatabase.getDatabase(), pDatabase.getPort()
							.toString());
					
					// Connect to the node and add the connection to the list
					NodeThread thread = new NodeThread(mNodeConnectionProvider.createConnection(node.getAddress(), node.getPort().toString()), node.getAddress(), db);
	...
	private void setUpDatabase(final DatabaseType pDatabase) {
		...
		// Setup the databse with the first entry in the list
		this.mServerDatabaseConnection.setUpDatabaseConnection(pDatabase.getUser(), pDatabase
				.getPassword(), pDatabase.getDatabase(), pDatabase.getPort()
				.toString());
	}
	...
	private class NodeThread implements Runnable {
		...
		public NodeThread(INodeConnection pConnection, String pNodeId, ClickWatchDB pDatabase){
			this.mConnection = pConnection;
			this.mMetaDataAdapter = this.mConnection.getAdapter(IMetaDataAdapter.class);
			this.mHandlerAdapter = this.mConnection.getAdapter(IHandlerAdapter.class);
		...
		public void run() {
			...
			mHandlerAdapter.configure(mMetaDataAdapter.pullAllMetaData().getAllHandlers());
			
			while(keepConnection){
				Collection<Handler> handlerCollection = this.mHandlerAdapter.pullHandler();
				// The method transforms a collection of handlers to a collection of handler records
				Collection<HandlerRecord> handlerRecords = Collections2.transform(handlerCollection, new Function<Handler, HandlerRecord>() {
					@Override
					public HandlerRecord apply(Handler pFrom) {
						HandlerRecord result = RecordFactory.createHandlerRecord();
						// TODO: Experiment ID
						result.setExperimentId("Foo");
						result.setNodeId(mNodeId);
						result.setQualifiedName(pFrom.getQualifiedName());
