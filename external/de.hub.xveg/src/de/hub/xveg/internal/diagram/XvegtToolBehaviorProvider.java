...
public class XvegtToolBehaviorProvider extends DefaultToolBehaviorProvider {
	
	public XvegtToolBehaviorProvider(IDiagramTypeProvider dtp, XvegDiagramType diagramType) {
		super(dtp);
	}
	...
	public IContextButtonPadData getContextButtonPad(IPictogramElementContext context) {
		...
		button.setIconId(XvegCompositionImageProvider.IMG_EREFERENCE);
	...
	public ICustomFeature getDoubleClickFeature(IDoubleClickContext context) {
		...
		if (pictogramElements.length > 0) {
			...
			List<EObject> bos = pictogramElement.getLink().getBusinessObjects();
			if (bos.size() == 1 && bos.get(0) instanceof EObject) {
				ICustomFeature[] possibleFeatures = new ICustomFeature[] { 
						new RenameElementFeature(featureProvider) 
				}; 
