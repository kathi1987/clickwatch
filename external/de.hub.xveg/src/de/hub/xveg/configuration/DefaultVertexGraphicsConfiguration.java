...
import de.hub.xveg.internal.features.DefaultStyle;
import de.hub.xveg.xvegfeaturemodel.StyleInfo;
import de.hub.xveg.xvegfeaturemodel.VertexFeature;

public class DefaultVertexGraphicsConfiguration implements IVertexGraphicConfiguration {
    ...
    protected void stylize(GraphicsAlgorithm shape) {
    	...
    	shape.setForeground(manageColor(DefaultStyle.createColorConstant(style.getLineColor(), DefaultStyle.SHAPE_FOREGROUND)));
    	shape.setBackground(manageColor(DefaultStyle.createColorConstant(style.getBackgroundColor(), DefaultStyle.SHAPE_BACKGROUND)));
	...
	public PictogramElement addGraphics(VertexFeature feature,
		...
		String label = DefaultVertexConfigurationProvider.INSTANCE
				.getLabelConfiguration(feature, dslObject).getLabel(feature, dslObject);
        
        Text text = gaService.createDefaultText(shape, label);
        text.setForeground(helper.manageColor(DefaultStyle.createColorConstant(style.getTextColor(), DefaultStyle.TEXT_FOREGROUND)));
