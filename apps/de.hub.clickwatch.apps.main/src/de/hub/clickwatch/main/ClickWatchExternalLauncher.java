...
public class ClickWatchExternalLauncher {
	...
	private ClickWatchExternalLauncher(
			Class<? extends IClickWatchMain> mainClass,
			Class<? extends IClickWatchContextAdapter>[] adapterClasses) {
	...
	private boolean initalize(String[] args) {
		...
		for (Class<? extends IClickWatchContextAdapter> adapterClass : adapterClasses) {
			adapters.put(adapterClass,
					adapterInjector.getInstance(adapterClass));
		}
		...
		try {
			...
			new HelpFormatter().printHelp(e.getLocalizedMessage(),
					commandLineOptions);
			return false;
		}
		return true;
	...
	public static void launch(String[] args,
			...
			Class<? extends IClickWatchContextAdapter>[] adapterClasses) {
		standaloneSetUp();
		ClickWatchExternalLauncher launcher = new ClickWatchExternalLauncher(
				mainClass, adapterClasses);
