function[] = software_models()
    ...
    function[] = plotSizes(LOC, years, at)
        ...
        hold on
        [AX, H1, H2] = plotyy(years, rv_LOC, years, LOC);
        datetick(AX(1),'x','mm/yy','keepticks');        
        datetick(AX(2),'x','mm/yy','keepticks');        
        plot(years, r_LOC,'-^');
        datetick('x','mm/yy','keepticks');
        xlim(AX(1), [years(1),years(end)]);
        xlim(AX(2), [years(1),years(end)]);
        set(AX(1), 'XTick', years(1):(years(end)-years(1))/4:years(end));
        set(AX(2), 'XTick', years(1):(years(end)-years(1))/4:years(end));
        set(H1,'Marker','x');
        set(H2,'Marker','o'); 
        
        set(get(AX(1), 'YLabel'),'String', 'model size in number of objects');
        set(get(AX(2), 'YLabel'),'String', 'SLOC');
